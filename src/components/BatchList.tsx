      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
        {filteredBatches.map((batch) => {
          ...
          return (
            <div
              key={batch.id}
              className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-5 transition-all hover:border-zinc-700 hover:bg-zinc-900 flex flex-col justify-between"
            >
              <div>
                {/* Header Row */}
                <div className="flex items-start justify-between gap-2">
                  ...
                  {batch.status === "available" && (
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-sm hover:bg-emerald-500/20">
                      Siap Diambil
                    </Badge>
                  )}
                  {batch.status === "matched" && (
                    <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/30 text-sm hover:bg-blue-500/20">
                      Terklaim
                    </Badge>
                  )}
                  {batch.status === "in_transit" && (
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30 text-sm hover:bg-amber-500/20">
                      Dalam Pengiriman
                    </Badge>
                  )}
                </div>
                ...
                <div className="mt-3.5 flex items-center gap-2.5">
                  <span className="rounded-md bg-zinc-800 px-2.5 py-1 text-sm font-semibold text-emerald-400 border border-zinc-700">
                    {batch.weightKg} kg
                  </span>
                ...
                {batch.notes && (
                  <p className="mt-2.5 text-sm text-zinc-400 line-clamp-1 italic">
                    &ldquo;{batch.notes}&rdquo;
                  </p>
                )}

                {/* Eco-Health Metrics Pill */}
                <div className="mt-3.5 rounded-lg bg-zinc-950/60 p-2.5 border border-zinc-800/80 flex items-center justify-between text-sm">
              ...
              <div className="mt-4 pt-3.5 border-t border-zinc-800/60 flex items-center justify-between">
                ...
                {batch.status === "available" ? (
                  <Button
                    ...
                    className="h-8 bg-emerald-600 hover:bg-emerald-500 text-white text-sm px-3.5"
                  >
                    Klaim Aliran
