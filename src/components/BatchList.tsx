      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredBatches.map((batch) => {
          ...
          return (
            <div
              key={batch.id}
              className="rounded-2xl border border-zinc-800/90 bg-zinc-900/60 p-6 sm:p-7 transition-all hover:border-zinc-700 hover:bg-zinc-900/80 flex flex-col justify-between shadow-sm"
            >
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  {batch.status === "available" && (
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-sm px-3 py-1 hover:bg-emerald-500/20">
                      Siap Diambil
                    </Badge>
                  )}
                  {batch.status === "matched" && (
                    <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/30 text-sm px-3 py-1 hover:bg-blue-500/20">
                      Terklaim
                    </Badge>
                  )}
                  {batch.status === "in_transit" && (
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30 text-sm px-3 py-1 hover:bg-amber-500/20">
                      Dalam Pengiriman
                    </Badge>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  <span className="rounded-md bg-zinc-800 px-3 py-1 text-sm font-semibold text-emerald-400 border border-zinc-700">
                    {batch.weightKg} kg
                  </span>
                ...
                <div className="rounded-xl bg-zinc-950/70 p-3.5 border border-zinc-800/80 flex items-center justify-between text-sm">
